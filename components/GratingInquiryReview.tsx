...
export default function GratingInquiryReview() {
  ...
  return <section aria-label="Imported grating requirement" className="rounded-card border border-teal-border bg-teal-bg p-5">
    ...
    <ul className="mt-4 space-y-3">{draft.lines.map(line => <li key={line.key} className="rounded-card bg-white p-3 text-sm"><strong>{line.mark} · {gratingCatalog.find(s => s.id === line.spec)?.title || 'Per drawing'}</strong><p className="mt-1">{line.length || '?'} × {line.width || '?'} {line.unit} · {line.quantity || '?'} panels</p></li>)}</ul>
