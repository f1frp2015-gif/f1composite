...
export default function ProductRfq({
  ...
  const advisor = `/ask?prefill=${encodeURIComponent(`I am evaluating ${product}. Help me choose the section, resin and standards, and list what F1 Composite needs for a qualified RFQ.`)}`;
  return (
    <div data-page-rfq className="grid grid-cols-1 gap-[28px] lg:grid-cols-[minmax(0,0.9fr)_minmax(0,1.1fr)] lg:gap-[56px]">
      <div>
        <p className="text-f16 leading-relaxed text-white/80">
          Send the size, length and quantity, the service environment and the destination. We reply within {supplyTerms.responseTime}; a formal quotation follows once the specification and delivery are reviewed.
        </p>
        <div className="mt-[20px] flex flex-wrap items-center gap-[10px]">
          <Button href={buildRfqHref({ source: "product-rfq", product, productPath })}>Request a quote</Button>
