...
export default function CaseStudyGrid({ items }: { items: CaseItem[] }) {
  ...
  return <>
          ...
          <p role="status" className="mb-[16px] text-f14 text-t2">{filtered.length} project and engineering references</p>
          {/* Grid */}
            ...
            {filtered.map((cs) => (
              ...
              >
                ...
                <div className="p-[21px]">
                  <h3 className="line-clamp-2 text-f18 font-bold text-t1 group-hover:text-teal-text">
                    {cs.title}
                  </h3>
                  <p className="mt-[8px] line-clamp-3 text-f14 leading-golden text-t2">{cs.excerpt}</p>
                  <div className="mt-[13px] flex gap-[13px] text-f12 text-t3">
