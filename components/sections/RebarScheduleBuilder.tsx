...
export default function RebarScheduleBuilder() {
  ...
  return <div className="rounded-card border border-border-default bg-white p-5 sm:p-8">
    ...
    <details className="mt-6 rounded-card bg-bg2 p-4" open={Boolean(error)}><summary className="cursor-pointer font-semibold text-t1">Review your inquiry summary</summary><pre className="mt-4 whitespace-pre-wrap break-words font-sans text-sm leading-relaxed text-t2">{rebarInquirySummary(value)}</pre></details>
    ...
    <div className="mt-6 flex flex-wrap items-center gap-4"><button type="button" onClick={continueToContact} className="min-h-12 rounded-control bg-teal-text px-6 py-3 text-sm font-bold text-white hover:bg-teal">Continue to contact form →</button><a href="/downloads/f1-rebar-schedule-template.csv" download className="inline-flex min-h-11 items-center text-sm font-bold text-teal-text underline underline-offset-4">Download a blank schedule (CSV)</a></div>
