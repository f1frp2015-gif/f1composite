...
export default function TubeSizeTable({ sizes, columns, product, productPath }: {
  ...
  return (
    <>
      <p id="tube-size-notes" className="mt-[13px] max-w-[850px] text-f13 leading-golden text-t2">
          ...
          <thead>
            <tr className="border-b-2 border-border-default bg-bg2">
              {["Model", ...columns, "Weight (kg/m)", "Select"].map((column) => (
                <th key={column} scope="col" className="whitespace-nowrap px-[16px] py-[13px] text-f13 font-bold text-t1">{column}</th>
              ))}
            </tr>
          </thead>
          <tbody>
                ...
                <tr key={size.model} className="border-b border-border-default last:border-b-0 hover:bg-bg2/60">
                  ...
                  {size.dimensions.map((dimension, index) => (
                    <td key={columns[index]} className="px-[16px] py-[13px] text-f15 text-t1">
                      ...
                      <span className="mt-[3px] block whitespace-nowrap text-f11 text-t3">≈ {approximateInches(dimension)} in</span>
                    </td>
                  ))}
                  <td className="px-[16px] py-[13px] text-f13 font-medium text-teal-text">{size.weight}</td>
                  <td className="px-[16px] py-[8px]">
                    <Link
                      href={tubeInquiryHref(productPath, product, specification)}
                      prefetch={false}
                      aria-label={`Quote ${size.model}`}
                      className="inline-flex min-h-[44px] items-center whitespace-nowrap rounded-[6px] border border-teal-border px-[14px] text-f13 font-semibold text-teal-text hover:bg-teal-bg focus-visible:outline-2 focus-visible:outline-offset-2 focus-visible:outline-teal"
      ...
      <p className="mt-[13px] text-f13 text-t2">Need a different size or an exact inch dimension? <Link href="/products/custom-pultruded-profiles" className="font-semibold text-teal-text underline">Send a custom section drawing</Link>.</p>
    </>
  );
}
