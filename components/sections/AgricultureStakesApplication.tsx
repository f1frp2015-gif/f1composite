  ...
  return (
    <>
      ...
      <Section id="procurement" tag="05 · A clear route to supply" title="From a field requirement to a repeatable order" shaded>
        <ol className="mt-8 grid gap-4 md:grid-cols-2 xl:grid-cols-5">{plantingSteps.map((step, index) => <li key={step.title} className="rounded-[10px] border border-border-default bg-white p-5"><span className="text-f24 font-extrabold text-teal-text">0{index + 1}</span><h3 className="mt-4 text-f18 font-bold leading-tight text-t1">{step.title}</h3><p className="mt-4 text-sm leading-relaxed text-t2">{step.buyer}</p><p className="mt-4 border-t border-border-default pt-4 text-sm leading-relaxed text-t2"><strong className="text-t1">Decision / output: </strong>{step.outcome}</p></li>)}</ol>
