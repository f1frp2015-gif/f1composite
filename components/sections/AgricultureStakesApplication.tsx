...
const wrap = "mx-auto max-w-[1280px] px-5 sm:px-7 lg:px-9";
const heading = "mt-3 text-[clamp(26px,3.3vw,40px)] font-extrabold leading-tight tracking-[-0.025em] text-t1";
const paragraph = "mt-4 text-f15 leading-relaxed text-t2";
  ...
  return (
    <>
      ...
      <section className="bg-white py-12 md:py-16">
        ...
        ].map(([title, body]) => <div key={title} className="rounded-[8px] bg-bg2 p-5"><h3 className="font-bold text-t1">{title}</h3><p className="mt-2 text-sm leading-relaxed text-t2">{body}</p></div>)}</div>
      </section>

      <Section id="growing-applications" tag="01 · Match the growing task" title="Where fiberglass stakes fit" shaded>
        <div className="mt-8 grid gap-5 md:grid-cols-2">{plantingApplications.map((item, index) => <article key={item.title} className="rounded-[10px] border border-border-default bg-white p-6 sm:p-7">
      ...
      <Section id="field-trial" tag="04 · Qualify before volume release" title="Installation, inspection & a useful field trial">
        <div className="mt-8 grid gap-9 lg:grid-cols-2"><div><figure><Image src={frpStakeImageAssets.nursery} alt="Illustrative young nursery tree supported with fiberglass stakes and broad soft ties" width={1536} height={1024} sizes="(max-width: 1024px) 100vw, 580px" className="h-auto w-full rounded-[10px]" /><figcaption className="mt-3 text-xs leading-relaxed text-t3">Nursery support visualization. Stake count and tie positions must suit the plant and site.</figcaption></figure><p className={paragraph}>For trees, first establish whether support is needed. Where staking is appropriate, allow movement and inspect ties as the trunk grows. Remove temporary support when the tree is established. <a href="https://www.extension.umd.edu/resource/planting-tree-or-shrub" target="_blank" rel="noopener noreferrer" className="font-semibold text-teal-text underline">University of Maryland Extension guidance</a>.</p></div>
          ...
          ].map(([title, body], index) => <li key={title} className="flex gap-4"><span className="flex h-9 w-9 shrink-0 items-center justify-center rounded-full bg-bg2 font-bold text-teal-text">{index + 1}</span><div><h3 className="text-f19 font-bold text-t1">{title}</h3><p className="mt-2 text-sm leading-relaxed text-t2">{body}</p></div></li>)}</ol>
        </div>
        <div className="mt-9 flex flex-col gap-5 rounded-[10px] bg-bg2 p-6 sm:flex-row sm:items-center sm:justify-between"><div><h3 className="text-f19 font-bold text-t1">Make the sample decision measurable.</h3><p className="mt-2 max-w-[740px] text-sm leading-relaxed text-t2">Agree the trial duration and acceptance criteria with your growing team. Short trials confirm fit and handling; they do not prove multi-year UV durability or storm performance.</p></div><Button href={buildAgricultureInquiry("sample")} className="shrink-0">Request trial samples</Button></div>
      </Section>

      <Section id="procurement" tag="05 · A clear route to supply" title="From a field requirement to a repeatable order" shaded>
        <ol className="mt-8 grid gap-4 md:grid-cols-2 xl:grid-cols-5">{plantingSteps.map((step, index) => <li key={step.title} className="rounded-[10px] border border-border-default bg-white p-5"><span className="text-f24 font-extrabold text-teal-text">0{index + 1}</span><h3 className="mt-4 text-f19 font-bold leading-tight text-t1">{step.title}</h3><p className="mt-4 text-sm leading-relaxed text-t2">{step.buyer}</p><p className="mt-4 border-t border-border-default pt-4 text-sm leading-relaxed text-t2"><strong className="text-t1">Decision / output: </strong>{step.outcome}</p></li>)}</ol>
