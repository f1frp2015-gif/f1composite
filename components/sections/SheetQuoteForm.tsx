...
const inputClass = "mt-[6px] min-h-[46px] w-full rounded-[6px] border border-border-default bg-white px-[12px] text-f15 text-t1 focus:outline-2 focus:outline-offset-2 focus:outline-teal";
  ...
  return (
    <form action="/contact" method="get" className="mt-[24px] rounded-[10px] border border-border-default bg-white p-[21px] sm:p-[28px]" aria-label="Prepare a fiberglass sheet inquiry">
      ...
      <div className="grid gap-[18px] sm:grid-cols-2 lg:grid-cols-3">
        <label htmlFor="sheet-unit" className="text-f13 font-semibold text-t1">Dimension units
        ...
        {([
          ...
          <label key={key} htmlFor={`sheet-${key}`} className="text-f13 font-semibold text-t1">
        ...
        <label htmlFor="sheet-surface" className="text-f13 font-semibold text-t1">Surface preference
      ...
      <p className="mt-[8px] text-f13 leading-golden text-t2">These are your requested part dimensions. We confirm the laminate, feasible sizes and production quantity after review. Add a drawing, service conditions and delivery details on the next page.</p>
      <div className="mt-[20px]"><Button type="submit">Continue to quote →</Button></div>
      <noscript><p className="mt-[12px] text-f13">Enable JavaScript to carry these dimensions into your inquiry, or use the product quotation link above and include them in your message.</p></noscript>
    </form>
  );
}
