...
export default function GratingProjectPlanner({ family }: { family?: 'molded' | 'pultruded' }) {
  ...
  function add(spec = '') {
    ...
    setNotice('Added to your panel schedule below.');
  ...
  return <section id="grating-planner" className="scroll-mt-24 bg-bg2 px-5 py-10 sm:px-7">
    <div className="mx-auto max-w-[1280px] space-y-6">
      ...
      {mode && <div className="space-y-6">
        ...
        {mode !== 'drawing' && <div id="grating-filter" className="rounded-xl bg-white p-5">
          ...
          <div className="grid max-h-[650px] gap-4 overflow-y-auto pr-1 md:grid-cols-2 lg:grid-cols-3" tabIndex={0} role="region" aria-label="Matching grating configurations">{matches.map(s => <article key={s.id} className="rounded-lg border border-border-default p-4">
        ...
        <div className="rounded-xl bg-deep p-5 text-white"><h3 className="text-xl font-bold">Review your RFQ summary</h3><p className="mt-2 text-sm">Unknowns stay visible for review. Continue to add contact details and upload your drawing; nothing is sent yet.</p><pre id="grating-print-summary" className="my-4 max-h-80 overflow-auto whitespace-pre-wrap break-words rounded bg-white p-4 font-sans text-sm text-t1">{summary}</pre><div className="flex flex-wrap gap-3"><button className="min-h-11 rounded bg-white px-4 py-3 font-bold text-deep" disabled={!ready} onClick={next}>Continue to inquiry & upload drawing →</button><button className="min-h-11 rounded border px-4" onClick={download}>Download summary</button><button className="min-h-11 rounded border px-4" onClick={() => window.print()}>Print summary</button></div><a className="mt-4 inline-block underline" href="/contact?source=grating-drawing&product=FRP+Grating&product_path=%2Fproducts%2Fgrating&inquiry_type=rfq">Open contact form without draft</a></div>
      </div>}
      <p role="status" className="text-sm text-teal-text">{notice}</p>
      {project.lines.length > 0 && <a href="#grating-project-schedule" onClick={() => { if (!mode) setMode('spec'); }} className="inline-flex min-h-11 items-center rounded-full bg-deep px-5 text-sm font-bold text-white">Your panel schedule: {project.lines.length} lines ↓</a>}
    </div>
  </section>;
}
