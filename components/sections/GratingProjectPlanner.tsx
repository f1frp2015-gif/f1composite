  ...
  return <section id="grating-planner" className="scroll-mt-24 bg-bg2 px-5 py-10 pb-28 sm:px-7 lg:pb-12">
    <div className="mx-auto max-w-[1280px] space-y-6"><span id="grating-configurations" className="block scroll-mt-24" /><span id="grating-quote" className="block scroll-mt-24" /><span id="grating-help" className="block scroll-mt-24" /><span id="grating-budget" className="block scroll-mt-24" /><span id="grating-review" className="block scroll-mt-24" />
      ...
      {mode && <>
        ...
        <div className="fixed inset-x-0 bottom-0 z-[60] flex items-center justify-between gap-3 border-t border-border-default bg-white px-5 py-3 pb-[max(12px,env(safe-area-inset-bottom))] shadow-lg lg:hidden"><span className="text-sm"><strong>{project.lines.length} panel {project.lines.length === 1 ? "line" : "lines"}</strong><br/>Step {step} of 3</span><button className={button} onClick={() => step < 3 ? goStep(step + 1) : next()}>{step < 3 ? 'Next step →' : 'Contact & upload →'}</button></div>
      </>}
