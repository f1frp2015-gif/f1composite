...
const input = 'mt-1 w-full rounded-md border border-border-default bg-white p-3 text-sm text-t1';
const button = 'inline-flex min-h-11 items-center justify-center rounded-md border border-teal-border px-4 py-2 text-sm font-bold text-teal-text disabled:opacity-40';
    ...
    <div className="mx-auto max-w-[1280px] space-y-6"><span id="grating-configurations" className="block scroll-mt-24" /><span id="grating-quote" className="block scroll-mt-24" /><span id="grating-help" className="block scroll-mt-24" /><span id="grating-budget" className="block scroll-mt-24" /><span id="grating-review" className="block scroll-mt-24" />
      ...
      {mode && <>
        ...
        {<div id="grating-filter" className="rounded-xl bg-white p-5">
          ...
          {matches.length === 0 && <p className="rounded-md bg-bg2 p-4">No configuration matches these filters. Reset a filter or use your drawing to request a review.</p>}
          <div className="grid max-h-[650px] gap-4 overflow-y-auto pr-1 md:grid-cols-2 xl:grid-cols-2" tabIndex={0} role="region" aria-label="Matching grating configurations">{matches.map(s => <article key={s.id} className="rounded-lg border border-border-default p-4">
            <div className="flex items-center gap-3"><Image src={photo[s.family as keyof typeof photo]} alt={`${s.family} construction reference, not an exact model photograph`} width={80} height={64} className="h-16 w-20 rounded object-cover" /><div><p className="text-xs uppercase text-teal-text">{s.family}</p><h4 className="font-bold">{s.title}</h4></div></div>
        ...
        <div className="rounded-xl bg-deep p-5 text-white"><h3 className="text-xl font-bold">Review your RFQ summary</h3><p className="mt-2 text-sm">Unknowns stay visible for review. Continue to add contact details and upload your drawing; nothing is sent yet.</p><div className="my-4 rounded-lg bg-white p-4 text-t1"><p className="font-bold">{project.fields.stage} · {project.lines.length} panel {project.lines.length === 1 ? "line" : "lines"}</p><p className="mt-2 text-sm">Delivery: {project.fields.destination || 'To confirm'}</p><ul className="mt-3 space-y-3 text-sm">{project.lines.map(line => <li key={line.key}><strong>{line.mark} · {gratingCatalog.find(s => s.id === line.spec)?.title || 'Per drawing'}</strong><p>{line.length || '?'} × {line.width || '?'} {line.unit} · {line.quantity || '?'} panels</p></li>)}</ul><button className="mt-3 min-h-11 font-bold text-teal-text underline" onClick={() => goStep(2)}>Edit sizes and requirements</button></div>
          <details><summary className="cursor-pointer py-3 text-sm">View full specification and engineering notes</summary><pre id="grating-print-summary" className="my-4 max-h-80 overflow-auto whitespace-pre-wrap break-words rounded bg-white p-4 font-sans text-sm text-t1">{summary}</pre></details><div className="flex flex-wrap gap-3"><button className="min-h-11 rounded bg-white px-4 py-3 font-bold text-deep" disabled={!ready} onClick={next}>Continue to inquiry & upload drawing →</button><button className="min-h-11 rounded border px-4" onClick={download}>Download summary</button><button className="min-h-11 rounded border px-4" onClick={() => {
