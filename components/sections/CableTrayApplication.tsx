...
const rfqHref = buildRfqHref({
  ...
  product: "FRP cable tray and support components",
  productPath: "/applications/frp-cable-tray-supports",
  message: "Please review my FRP cable tray / support enquiry. Scope (support profiles, fabricated supports, or complete tray system): __. Tray type, width, depth and route length: __. Cable mass and future allowance: __. Support spacing and fixing structure: __. Chemicals and operating temperature: __. Fire / electrical / project standards: __. Fittings, covers and fasteners: __. Quantity, destination and required date: __. Drawings can follow.",
});
...
const sources = {
  ...
  eaton: { label: "Eaton · fiberglass system selection", href: "https://www.eaton.com/content/dam/eaton/products/support-systems/cable-management/cable-tray-management-catalog.pdf" },
...
const navigation = [
  ["system-types", "System types"], ["applications", "Applications"], ["supports", "Supports & fittings"],
...
export default function CableTrayApplication({ page }: { page: ApplicationPage }) {
  return <>
    <PageHeader tag="Cable management · application & selection guide" title={page.h1} description={page.description}
      breadcrumbs={[{ label: "Home", href: "/" }, { label: "Applications", href: "/applications" }, { label: "FRP cable trays & supports" }]}
    ...
    <section className="bg-white pt-[34px] pb-[42px]">
      <div className="mx-auto max-w-[1280px] px-[20px] sm:px-[34px]">
        <figure className="overflow-hidden rounded-[12px] border border-border-default bg-bg2">
          <Image src={page.image} alt={page.imageAlt} width={1672} height={941} sizes="(max-width: 1280px) 100vw, 1212px" className="h-auto w-full" preload />
          <figcaption className="px-[20px] py-[12px] text-f13 leading-relaxed text-t3">{page.imageCaption}</figcaption>
        </figure>
    ...
    <Section id="system-types" tag="01 / Select the system" title="Match the containment to the cables">
      <p className="max-w-[920px]">Cable ladder, ventilated tray and enclosed trunking solve different routing problems. A cover does not automatically turn a tray into a sealed enclosure. Select cable support, ventilation, bend radius and access together.</p>
    ...
    <Section id="supports" tag="03 / Complete the load path" title="The support and connection deserve as much attention as the tray">
      <div className="grid items-start gap-[30px] lg:grid-cols-2">
        <figure className="overflow-hidden rounded-[10px] border border-border-default bg-bg2"><Image src="/images/applications/frp-cable-tray-wall-support-pultruded-texture.webp" alt="Illustrative fiberglass cable ladder on a wall-mounted crossarm with a gusset, hold-down clips and bolted wall attachment" width={1536} height={1024} sizes="(max-width: 1024px) 100vw, 590px" className="h-auto w-full" /><figcaption className="px-[18px] py-[12px] text-f13 text-t3">AI-generated support concept. Hole patterns, anchors, clamps and member sizes require an approved project detail.</figcaption></figure>
        ...
        ].map(([title, body]) => <div key={title}><h3 className="text-f19 font-bold text-t1">{title}</h3><p className="mt-[8px]">{body}</p></div>)}</div>
      </div>
    ...
    <Section id="specification" tag="08 / Quote-ready scope" title="Turn a route drawing into a comparable quotation" muted>
      <Table caption="Project information for FRP cable trays and supports" headers={["Input", "Information to send", "Why it matters"]} rows={[
        ...
        ["Geometry", "Route drawing, width, loading depth, length, bends, risers, reducers and minimum cable bend radius.", "Defines straight sections, fittings and usable space."],
    ...
    <section id="questions" className="scroll-mt-[110px] bg-white px-[20px] pt-[5px] pb-[55px] sm:px-[34px]"><div className="mx-auto max-w-[1212px]"><FAQ title="FRP cable tray questions, answered" items={faqs} /></div></section>

    <Section id="references" tag="Technical references" title="Further reading for engineers and specifiers" muted>
      <p className="max-w-[900px]">This guide draws on public system documentation from Eaton, Enduro / Creative Composites Group, Øglænd, Mita / Wibe Group and Niedax Ebo, alongside standards publishers. Their product ratings and approvals apply to their own systems. References checked September 21, 2026.</p>
