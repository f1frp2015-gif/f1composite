  ...
  return (
    <section className="border-y border-border-default bg-bg2 py-[36px]" aria-label="Specification and quotation next steps">
      <div className="site-container">
        ...
        <p className="mt-[8px] max-w-[850px] text-f14 text-t2">{commercialFacts.availability}</p>
        <div className="mt-[22px] grid gap-[24px] md:grid-cols-3">
          <div><h3 className="font-bold text-t1">1. Check the application</h3><Link className="mt-[8px] block text-f14 text-teal-text underline underline-offset-4" href={journey.application}>{journey.applicationLabel}</Link><Link className="mt-[8px] block text-f14 text-teal-text underline underline-offset-4" href={journey.tool}>{journey.toolLabel}</Link>{priceEstimator && <Link className="mt-[8px] block text-f14 text-teal-text underline underline-offset-4" href="/fiberglass-pultruded-profile-price">Estimate a fiberglass pultruded profile price</Link>}</div>
          <div><h3 className="font-bold text-t1">2. Review supporting evidence</h3><Link className="mt-[8px] block text-f14 text-teal-text underline underline-offset-4" href={document ? `/resources/evidence#${document.id}` : "/resources/evidence"}>{document ? `${document.title} and scope` : "Find applicable product evidence"}</Link><Link className="mt-[8px] block text-f14 text-teal-text underline underline-offset-4" href={journey.project}>{journey.projectLabel}</Link><p className="mt-[8px] text-f12 text-t3">Confirm that the document and project match your proposed product.</p></div>
          <div><h3 className="font-bold text-t1">3. Send the specification</h3><p className="mt-[8px] text-f14 text-t2">Include the drawing, resin or environment, loads, quantity and destination.</p><Link className="mt-[12px] inline-flex min-h-[44px] items-center rounded-control bg-teal-text px-[16px] text-f14 font-bold text-white" href={buildRfqHref({ source: "product-next-steps", product: journey.title, productPath: path })}>Request a specification review</Link></div>
        </div>
      </div>
    </section>
  );
