...
export default function GratingSelectionCriteria() {
  return <section id="grating-selection" className="scroll-mt-[100px] border-y border-border-default bg-bg2 py-[44px] md:py-[60px]">
    <div className="mx-auto max-w-[1280px] px-[20px] sm:px-[28px] lg:px-[34px]">
      <p className="text-f11 font-bold uppercase tracking-[0.12em] text-teal-text">Before choosing a thickness</p>
      <h2 className="mt-[10px] text-f31 font-bold text-t1">Specify the opening, surface and service conditions</h2>
      <div className="mt-[24px] grid gap-[24px] lg:grid-cols-[0.9fr_1.1fr]">
        <figure className="rounded-[10px] border border-border-default bg-white p-[22px]">
          ...
          <figcaption className="mt-[12px] text-f13 leading-relaxed text-t2">A mesh designation or bar-center dimension is not the unobstructed opening. Confirm the top clear opening and direction of travel where footwear, dropped objects or accessibility govern.</figcaption>
        </figure>
        <div className="space-y-[20px]">
          <article><h3 className="text-f19 font-bold text-t1">Match the resin to the environment</h3><p className="mt-[8px] text-f14 leading-relaxed text-t2">Identify chemical names, concentrations, exposure type and operating temperature. Discuss polyester or vinyl ester against that service; resin names alone do not establish compatibility.</p><Link className="mt-[8px] inline-block text-f13 font-bold text-teal-text underline underline-offset-4" href="/technology/pultrusion-resin-systems">Read about FRP resin systems →</Link></article>
          <article><h3 className="text-f19 font-bold text-t1">Choose the walking surface</h3><p className="mt-[8px] text-f14 leading-relaxed text-t2">Compare concave, fine-grit and coarse-grit options for wet or dry use, footwear and cleaning. Specify the required slip-test method and acceptance criteria; an anti-slip finish name is not a test report.</p></article>
          <article><h3 className="text-f19 font-bold text-t1">Check the full load case</h3><p className="mt-[8px] text-f14 leading-relaxed text-t2">State clear span, support width, uniform and concentrated loads, contact area and deflection limit. Cutouts, edge support and bearing-bar direction belong on the panel layout.</p><Link className="mt-[8px] inline-block text-f13 font-bold text-teal-text underline underline-offset-4" href="#grating-engineering">Request the applicable load data →</Link></article>
        </div>
      </div>
    </div>
  </section>;
