...
const variants: Record<Variant, string> = {
  solid: "rounded-control bg-deep text-white hover:bg-teal-text",
  outline: "rounded-control border border-border-default bg-white text-t1 hover:border-teal-border hover:text-teal-text",
};
...
export default function WhatsAppButton({
  ...
  return (
    <a
      ...
      className={`inline-flex min-h-[46px] items-center justify-center gap-[7px] text-f14 font-bold transition-colors focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-teal focus-visible:ring-offset-2 ${padding} ${variants[variant]} ${className}`}
    >
      <WhatsAppIcon size={iconOnly ? 22 : 18} className={variant === "outline" ? "text-whatsapp" : undefined} />
