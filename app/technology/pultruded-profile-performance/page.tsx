      ...
      <aside className="min-w-0">
        <nav aria-label="Performance page contents" className="rounded-xl border border-border-default bg-bg2 p-5 lg:sticky lg:top-[110px]">
          ...
          <ol className="mt-3 grid gap-1 sm:grid-cols-2 lg:grid-cols-1">
            {performanceSections.map((section, index) => <li key={section.id}><a className="flex min-h-11 items-center gap-3 rounded px-2 py-2 text-f14 font-semibold text-t1 hover:bg-white hover:text-teal-text" href={`#${section.id}`}><span className="text-teal-text">0{index + 1}</span>{section.short}</a></li>)}
      ...
      <div className="min-w-0">
          ...
          <div role="region" aria-label={`${section.title} table, scroll horizontally`} tabIndex={0} className="mt-3 overflow-x-auto rounded-lg border border-border-default focus-visible:outline-2 focus-visible:outline-teal-text sm:mt-5">
            <table className="w-full min-w-[780px] border-collapse text-f14">
              ...
              <tbody>{section.rows.map(row => <tr key={row.property} className="border-b border-border-default last:border-0 even:bg-bg2/60">
                ...
                <td className="p-4 align-top"><span className={`mb-2 inline-block rounded px-2 py-1 text-f12 font-semibold ${row.basis === "Published reference" ? "bg-teal-bg2 text-teal-text" : "bg-bg2 text-t2"}`}>{row.basis}</span><span className="block text-t1">{row.reference}</span></td>
        ...
        <section id="standards" className="mb-14 scroll-mt-[115px]">
          ...
          <div className="mt-5 grid gap-4 sm:grid-cols-2">
            ...
            ].map(card => <article key={card.title} className="rounded-xl border border-border-default bg-bg2 p-5"><h3 className="text-f18 font-bold">{card.title}</h3><p className="mt-3 text-f14 text-t2">{card.text}</p><SourceLinks ids={card.ids} /></article>)}
          </div>
        </section>
