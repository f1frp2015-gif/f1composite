...
function SourceLinks({ ids }: { ids: PerformanceSourceId[] }) {
  return <p className="mt-4 text-f13 leading-relaxed text-t2">
...
function DimensionSketch() {
  return <figure className="rounded-xl border border-border-default bg-white p-5">
    ...
    <figcaption className="mt-3 text-f13 text-t2">b = width · h = height · t = wall thickness. L follows the continuous fibers; transverse and through-thickness properties require separate data. Schematic, not to scale.</figcaption>
  </figure>;
}

function ChemicalComparison() {
  return <div className="mt-7 rounded-xl border border-border-default bg-bg2 p-5 sm:p-6">
    <h3 className="text-f19 font-bold">How resin and temperature change a screening result</h3>
    <p className="mt-2 text-f13 text-t2">Selected NHC supplier-chart examples, reproduced as a comparison reference. ISO here means <strong>isophthalic polyester</strong>; VE means vinyl ester. These are not F1 test results or approved service limits.</p>
    <div role="region" aria-label="Chemical screening examples, scroll horizontally" tabIndex={0} className="mt-4 overflow-x-auto rounded-lg focus-visible:outline-2 focus-visible:outline-teal-text">
      <table className="w-full min-w-[580px] border-collapse text-f13">
    ...
    <p className="mt-3 text-f13 text-t2">+ Good resistance in the source chart · 0 Possible attack; consider another resin · − Unsuitable in the source chart. The chart does not provide a complete exposure duration, stress or laminate specification; confirm compatibility for the actual supply.</p>
...
function GradeComparison() {
  return <div className="mt-7 rounded-xl border border-border-default bg-bg2 p-5 sm:p-6">
    <h3 className="text-f19 font-bold">E17 / E23: selected reference minimums</h3>
    <p className="mt-2 text-f13 text-t2">Selected EN 13706-3 grade requirements, cross-checked against Fiberline’s published comparison. These are standard reference minimums, not F1 measured results or design allowables. L = longitudinal; T = transverse.</p>
    <div role="region" aria-label="E17 and E23 grade comparison, scroll horizontally" tabIndex={0} className="mt-4 overflow-x-auto rounded-lg focus-visible:outline-2 focus-visible:outline-teal-text">
      <table className="w-full min-w-[460px] border-collapse text-f13">
...
export default function PultrudedProfilePerformancePage() {
  return <>
    ...
    <section className="border-b border-border-default bg-white py-8 sm:py-10">
      <div className="mx-auto grid max-w-[1280px] gap-6 px-5 sm:px-7 md:grid-cols-3">
        {[
        ...
        ].map(item => <div key={item.label}><p className="text-f13 font-bold text-teal-text">{item.label}</p><h2 className="mt-2 text-f19 font-bold">{item.title}</h2><p className="mt-2 text-f13 text-t2">{item.body}</p></div>)}
      </div>
    </section>

    <div className="mx-auto grid max-w-[1280px] gap-8 px-5 py-10 sm:px-7 lg:grid-cols-[210px_minmax(0,1fr)] lg:gap-10">
      <aside className="min-w-0">
        <nav aria-label="Performance page contents" className="rounded-xl border border-border-default bg-bg2 p-5 lg:sticky lg:top-[110px]">
          <p className="text-f13 font-bold uppercase tracking-wider text-t2">On this page</p>
          <ol className="mt-3 grid gap-1 sm:grid-cols-2 lg:grid-cols-1">
            {performanceSections.map((section, index) => <li key={section.id}><a className="flex min-h-11 items-center gap-3 rounded px-2 py-2 text-f13 font-semibold text-t1 hover:bg-white hover:text-teal-text" href={`#${section.id}`}><span className="text-teal-text">0{index + 1}</span>{section.short}</a></li>)}
            <li><a href="#standards" className="block px-2 py-3 text-f13 font-semibold text-t1 hover:text-teal-text">Standards & scope</a></li>
            <li><a href="#specification" className="block px-2 py-3 text-f13 font-semibold text-t1 hover:text-teal-text">Specification checklist</a></li>
            <li><a href="#sources" className="block px-2 py-3 text-f13 font-semibold text-t1 hover:text-teal-text">Source library</a></li>
          </ol>
        </nav>
      </aside>

      <div className="min-w-0">
        <div className="mb-10 rounded-xl border border-teal-border bg-teal-bg p-5 sm:p-6">
          <h2 className="text-f19 font-bold">How to read these tables</h2>
          <p className="mt-2 text-f15 text-t2">Published references illustrate the property and its units. Product-specific values and drawing tolerances must be confirmed for the offered configuration. References to a standard do not assert certification or compliance.</p>
          <p className="mt-3 text-f13 text-t2">A standard minimum, a typical published value, a measured batch result and a design allowable have different meanings. Use the <Link href="/resources/technical-data" className="font-semibold text-teal-text underline">material data page</Link> for the existing E23 table, and the <Link href="/resources/evidence" className="font-semibold text-teal-text underline">evidence library</Link> for available documents.</p>
        </div>
        ...
        {performanceSections.map((section, index) => <section key={section.id} id={section.id} aria-labelledby={`${section.id}-heading`} className="mb-14 scroll-mt-[115px]">
          ...
          <h2 id={`${section.id}-heading`} className="mt-3 text-f31 font-bold tracking-tight text-t1">{section.title}</h2>
          <p className="mt-3 text-f15 leading-relaxed text-t2">{section.intro}</p>
          {section.id === "dimensions" ? <div className="mt-5"><DimensionSketch /></div> : null}
          <p className="mt-5 text-f13 text-t3 sm:hidden">Scroll the table horizontally to view methods and conditions.</p>
          <div role="region" aria-label={`${section.title} table, scroll horizontally`} tabIndex={0} className="mt-3 overflow-x-auto rounded-lg border border-border-default focus-visible:outline-2 focus-visible:outline-teal-text sm:mt-5">
            <table className="w-full min-w-[780px] border-collapse text-f13">
              ...
              <tbody>{section.rows.map(row => <tr key={row.property} className="border-b border-border-default last:border-0 even:bg-bg2/60">
                <th scope="row" className="p-4 text-left align-top font-semibold text-t1">{row.property}<span className="mt-2 block text-f13 font-normal text-t2">{row.unit}</span></th>
                <td className="p-4 align-top"><span className={`mb-2 inline-block rounded px-2 py-1 text-[12px] font-semibold ${row.basis === "Published reference" ? "bg-teal-bg2 text-teal-text" : "bg-bg2 text-t2"}`}>{row.basis}</span><span className="block text-t1">{row.reference}</span></td>
                <td className="p-4 align-top text-t2">{row.methods}</td>
                <td className="p-4 align-top text-t2">{row.conditions}</td>
              </tr>)}</tbody>
            </table>
          </div>
          <p className="mt-4 border-l-[3px] border-teal pl-4 text-f15 leading-relaxed text-t2">{section.takeaway}</p>
          {section.id === "fire" ? <p className="mt-4 rounded-lg bg-bg2 p-4 text-f13 text-t2"><strong>China edition check · 21 September 2026:</strong> GB 8624-2012 remains current. GB 8624-2025 is published and takes effect on <strong>1 January 2027</strong>. Confirm the edition required for the project approval date. <a href={performanceSources.gbFire2025.href} className="text-teal-text underline">SAMR implementation record</a>.</p> : null}
        ...
        <section id="standards" className="mb-14 scroll-mt-[115px]">
          ...
          <h2 className="mt-3 text-f31 font-bold tracking-tight">Choose a specification route</h2>
          <p className="mt-3 text-f15 text-t2">These documents have different jobs. Select the governing product specification, then the relevant property tests and end-use requirements. Listing several methods does not make their results interchangeable.</p>
          <div className="mt-5 grid gap-4 sm:grid-cols-2">
            ...
            ].map(card => <article key={card.title} className="rounded-xl border border-border-default bg-bg2 p-5"><h3 className="text-f19 font-bold">{card.title}</h3><p className="mt-3 text-f13 text-t2">{card.text}</p><SourceLinks ids={card.ids} /></article>)}
          </div>
        </section>

        <section id="specification" className="mb-14 scroll-mt-[115px] rounded-xl bg-deep p-6 text-white sm:p-8">
          <p className="text-f13 font-semibold text-white/75">FROM REFERENCE TO RFQ</p>
          <h2 className="mt-3 text-f24 font-bold">Build a testable specification</h2>
          <ol className="mt-5 space-y-3 text-f15 text-white/85">
          ...
          <Link href={rfqHref} className="mt-6 inline-flex min-h-11 items-center rounded-lg bg-white px-5 py-3 font-bold text-deep hover:bg-bg2">Send My Performance Requirements →</Link>
        </section>
        ...
        <section id="sources" className="scroll-mt-[115px]">
          ...
          <p className="mt-3 text-f13 text-t2">Reviewed 21 September 2026. Public catalogue records establish document identity and scope; use the licensed standard and the agreed edition for contractual limits and complete procedures. Supplier and industry examples remain separately attributed.</p>
          <details className="mt-5 rounded-xl border border-border-default bg-bg2 p-5">
            <summary className="cursor-pointer text-f15 font-bold text-t1">Browse all {Object.keys(performanceSources).length} references</summary>
            <ul className="mt-5 grid gap-5 sm:grid-cols-2">{Object.entries(performanceSources).map(([id, source]) => <li key={id}><a className="text-f13 font-semibold text-teal-text underline underline-offset-4" href={source.href}>{source.label}</a><p className="mt-1 text-f13 text-t2">{source.note}</p></li>)}</ul>
          </details>
          <div className="mt-6 flex flex-wrap gap-x-6 gap-y-3 text-f13 font-semibold text-teal-text">
