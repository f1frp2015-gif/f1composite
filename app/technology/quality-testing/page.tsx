...
export default function QualityTestingPage() {
  return <>
    ...
    <section className="bg-bg2 py-[40px]"><div className="site-container">
      ...
      <p className="mt-[13px] max-w-[950px] text-f16 leading-relaxed text-t2">{commercialFacts.compliance}</p>
      <p className="mt-[13px] max-w-[950px] text-f16 leading-relaxed text-t2">A quality-management certificate describes a management-system scope; it does not establish the performance of every product. Request the current holder and scope for the proposed supply. Published technical references do not replace a project approval or a batch inspection report.</p>
      <Link className="mt-[20px] inline-flex min-h-[44px] items-center rounded-control bg-teal-text px-[20px] font-bold text-white" href="/resources/evidence">Browse reports and supporting documents</Link>
      <div className="mt-[34px] grid gap-[20px] md:grid-cols-2">{checks.map((check) => <article key={check.title} className="rounded-card border border-border-default bg-white p-[24px]"><h2 className="text-f18 font-bold text-t1">{check.title}</h2><p className="mt-[10px] text-f16 leading-relaxed text-t2">{check.text}</p></article>)}</div>
      <h2 className="mt-[40px] text-f24 font-bold text-t1">Available public references</h2>
      <ul className="mt-[16px] space-y-[14px]">{engineeringEvidence.map((record) => <li key={record.id}><Link href={`/resources/evidence#${record.id}`} className="font-semibold text-teal-text underline">{record.title}</Link><p className="mt-[4px] text-f14 text-t2">{record.scope}</p></li>)}</ul>
      <p className="mt-[20px] text-f16 text-t2">Product pages carry their own evidence and release scope, for example the laminate test data for <Link href="/products/wind-turbine-blade-panels" className="font-semibold text-teal-text underline">pultruded wind turbine blade panels</Link> and the inspection and release records for <Link href="/products/frp-pultrusion-manufacturer-factory-direct" className="font-semibold text-teal-text underline">factory-direct pultrusion supply</Link>.</p>
      <h2 className="mt-[40px] text-f24 font-bold text-t1">Include the acceptance requirements in your RFQ</h2>
      <p className="mt-[12px] text-f16 text-t2">Send the drawing, service conditions, required standard and edition, sampling plan and document requirements. We will confirm the available evidence and any testing gaps for your proposed product.</p>
