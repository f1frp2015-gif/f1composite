  ...
  return <>
      ...
      <div className={`${wrap} grid items-center gap-8 lg:grid-cols-[1.2fr_1fr]`}>
        <figure className="min-w-0">
          <a href={page.image} aria-label="View full-size fiberglass door frame profile rendering">
            <Image src={page.image} alt="Three fiberglass door frame profiles with continuous hollow raised stops running the full length: two open-back frame sections and one closed mullion concept" width={1536} height={1024} sizes="(max-width: 1024px) 100vw, 55vw" preload className="h-auto w-full rounded-card" />
          </a>
        ...
        <div>
          ...
          <h2 className="mt-4 text-f32 font-bold leading-tight text-t1">A fiberglass door frame starts with the section</h2>
          <p className="mt-4 text-f16 leading-relaxed text-t2">Pultrusion produces a constant cross-section along the profile length. That makes the section drawing the starting point for jamb depth, the door stop and seal interfaces. Cutting, joining and local hardware preparation turn those lengths into a frame.</p>
          <p className="mt-4 text-f16 leading-relaxed text-t2">The raised stop is a continuous part of the section. Its hollow passage runs in the same direction as the main profile; it is not a short tab attached near the end. Local pockets or stop terminations are made during secondary fabrication.</p>
          <p className="mt-4 text-f16 leading-relaxed text-t2">Specify the door leaf and wall connection alongside the profile. This lets the fabrication review address corner joints, anchor access and concentrated loads from hinges or closers before tooling is agreed.</p>
          <div className="mt-6 rounded-card bg-bg2 p-6"><h3 className="text-f16 font-bold text-t1">What F1 supplies</h3><p className="mt-2 text-f14 leading-relaxed text-t2">{page.scope}</p></div>
        </div>
      </div>
    </section>
    ...
    <section id="sections" className="scroll-mt-[150px] bg-bg2 py-12 md:py-16">
      <div className={wrap}>
        ...
        <h2 className="mt-4 text-f32 font-bold text-t1">Read the section before comparing sizes</h2>
        <p className="mt-4 max-w-3xl text-f16 leading-relaxed text-t2">Overall jamb depth spans the wall direction. Face width describes the visible frame leg; throat width is the clear opening at the back of an open section. These dimensions are related, but they are not interchangeable.</p>
        <div className="mt-7 grid gap-6 lg:grid-cols-3">
          {page.profileOptions.map((item) => <article key={item.title} className="overflow-hidden rounded-card border border-border-default bg-white">
        ...
        <div className="mt-8 grid gap-7 lg:grid-cols-[1fr_1.25fr]">
          <div>
            ...
            <p className="mt-4 text-f16 leading-relaxed text-t2">These example dimensions help explain the illustrated section family. They are not an F1 stocked-size list or tooling confirmation. Metric values are rounded-tag conversions of the inch reference dimensions.</p>
    ...
    <section className="bg-bg2 py-12 md:py-16">
      <div className={wrap}>
        ...
        <h2 className="mt-4 text-f32 font-bold text-t1">Coordinate every side of the opening</h2>
        <div className="mt-7 grid gap-5 md:grid-cols-2 lg:grid-cols-4">{page.components.map((item, index) => <article key={item.title} className="rounded-card border border-border-default bg-white p-6"><p className="text-f14 font-bold text-teal-text">0{index + 1}</p><h3 className="mt-4 text-f20 font-bold text-t1">{item.title}</h3><p className="mt-3 text-f14 leading-relaxed text-t2">{item.body}</p></article>)}</div>
    ...
    <section id="assembly" className="scroll-mt-[150px] bg-white py-12 md:py-16">
      <div className={wrap}>
        ...
        <h2 className="mt-4 text-f32 font-bold text-t1">From straight lengths to a fabricated frame</h2>
        <div className="mt-7 grid gap-6 md:grid-cols-2">{page.assembly.map((item) => <article key={item.title} className="rounded-card border border-border-default p-6"><h3 className="text-f20 font-bold text-t1">{item.title}</h3><p className="mt-3 text-f16 leading-relaxed text-t2">{item.body}</p></article>)}</div>
      </div>
    </section>
    ...
    <section id="anchoring" className="scroll-mt-[150px] bg-bg2 py-12 md:py-16">
      <div className={wrap}>
        ...
        <h2 className="mt-4 text-f32 font-bold text-t1">Select the anchor approach around the wall</h2>
        <p className="mt-4 max-w-3xl text-f16 leading-relaxed text-t2">An existing finished opening and a wall being built around the frame need different fixing details. Identify the substrate, construction stage, fixing access and service exposure before selecting the section.</p>
        <div className="mt-7 grid gap-6 md:grid-cols-2">{page.anchors.map((item) => <article key={item.title} className="rounded-card border border-border-default bg-white p-6"><h3 className="text-f20 font-bold text-t1">{item.title}</h3><p className="mt-3 text-f14 font-semibold text-teal-text">{item.method}</p><p className="mt-3 text-f14 leading-relaxed text-t2">{item.detail}</p></article>)}</div>
        <p className="mt-5 text-f14 leading-relaxed text-t3">Anchor layouts, sizes and spacing require project-specific design. The approaches above describe interfaces to review, not an installation schedule.</p>
      </div>
    </section>
    ...
    <section className="bg-white py-12 md:py-16">
      <div className={wrap}>
        ...
        <h2 className="mt-4 text-f32 font-bold text-t1">Where to consider an FRP door frame</h2>
        <div className="mt-7 grid gap-x-10 gap-y-7 md:grid-cols-2">{page.applications.map((item) => <article key={item.title} className="border-t border-border-default pt-5"><h3 className="text-f20 font-bold text-t1">{item.title}</h3><p className="mt-3 text-f16 leading-relaxed text-t2">{item.body}</p></article>)}</div>
      </div>
    </section>
    ...
    <section id="specification" className="scroll-mt-[110px] bg-bg2 py-12 md:py-16">
      <div className={wrap}>
        ...
        <h2 className="mt-4 text-f32 font-bold text-t1">Specify the frame before selecting the tooling</h2>
        <p className="mt-4 max-w-3xl text-f16 leading-relaxed text-t2">Use these inputs to develop a quote for your fiberglass door frame profile. Dimensions and tolerances belong on the agreed section drawing; assembly requirements belong in the opening and hardware schedule.</p>
        <dl className="mt-7 divide-y divide-border-default rounded-card border border-border-default bg-white px-6">{page.specification.map(([title, body]) => <div key={title} className="grid gap-2 py-5 md:grid-cols-[220px_1fr]"><dt className="text-f14 font-bold text-t1">{title}</dt><dd className="text-f14 leading-relaxed text-t2">{body}</dd></div>)}</dl>
    ...
    <section id="request-quote" className="scroll-mt-[110px] bg-white py-12 md:py-16">
      <div className={`${wrap} grid gap-8 lg:grid-cols-2`}>
        <div><SectionTag>Drawing review & quotation</SectionTag><h2 className="mt-4 text-f32 font-bold leading-tight text-t1">Send your door frame requirements</h2><p className="mt-4 text-f16 leading-relaxed text-t2">Start with an existing section or an opening concept. We review the profile geometry and supply scope, then define tooling and samples for evaluation before production.</p><Button href={quote} className="mt-6">Request a Door Frame Quote</Button></div>
        <div className="rounded-card border border-border-default bg-bg2 p-7"><h3 className="text-f20 font-bold text-t1">Include with your inquiry</h3><ul className="mt-4 list-disc space-y-3 pl-5 text-f16 text-t2">{page.checklist.map((item) => <li key={item}>{item}</li>)}</ul></div>
      </div>
    </section>
