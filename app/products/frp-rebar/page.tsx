...
export default function RebarPage() {
  ...
  return <>
    ...
    <ProductSection id="request-quote" title="A clear schedule makes the next step easier" tone="muted" intro="Use the optional builder, download a blank template, or send an existing BOQ directly. You do not need complete specifications to start a conversation."><RebarScheduleBuilder /><div className="mt-5 flex flex-wrap gap-x-6 text-sm"><Link href={buildRebarRfqHref()} className="inline-flex min-h-11 items-center font-bold text-teal-text">Already have a schedule? Go directly to the contact form →</Link><Link href="/tools/gfrp-rebar-calculator" className="inline-flex min-h-11 items-center font-bold text-teal-text">Match steel bar sizes and ACI 440.11 values →</Link></div><FAQ items={[...catalog.faqs]} title="FRP rebar purchasing questions" /></ProductSection>
