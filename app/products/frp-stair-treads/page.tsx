    ...
    <>
      ...
      <section className="bg-white py-[55px] md:py-[89px]">
        <div className="site-container">
          ...
          <div className="mt-[34px] grid gap-[13px] md:grid-cols-2">
            {decisionChecks.map(([label, title, body], index) => <article key={label} className="rounded-[8px] border border-border-default bg-bg2 p-[21px] sm:p-[24px]"><p className="text-f12 font-bold uppercase tracking-[.1em] text-teal-text">{index + 1} · {label}</p><h3 className="mt-[8px] text-f18 font-bold text-t1">{title}</h3><p className="mt-[10px] text-f14 leading-golden text-t2">{body}</p></article>)}
          </div>
        </div>
      </section>
      ...
      <section id="stair-tread-specifications" className="scroll-mt-[88px] bg-bg2 py-[55px] md:py-[89px]">
        <div className="site-container">
          ...
          <p className="mt-[21px] rounded-[8px] border border-amber-200 bg-amber-50 px-[18px] py-[15px] text-f14 leading-golden text-t2"><strong className="text-t1">Release boundary:</strong> covers rely on the existing step. Complete grating treads require an approved load/span check, support detail and fabrication drawing.</p>
      ...
      <section className="bg-white py-[55px] md:py-[89px]">
        <div className="site-container grid gap-[34px] lg:grid-cols-[1.05fr_.95fr] lg:items-center">
          ...
          <aside className="rounded-[8px] border border-border-default bg-bg2 p-[21px] sm:p-[34px]"><p className="text-f12 font-bold uppercase tracking-[.1em] text-teal-text">RFQ checklist</p><h3 className="mt-[8px] text-f24 font-bold text-t1">Eight inputs for a qualified quote</h3><ol className="mt-[21px] space-y-[11px]">{measurementInputs.map((input, index) => <li key={input} className="flex gap-[11px] text-f14 leading-golden text-t2"><span className="flex h-[24px] w-[24px] shrink-0 items-center justify-center rounded-full bg-white text-f12 font-bold text-teal-text">{index + 1}</span><span>{input}</span></li>)}</ol><Button href="/contact?source=stair-tread-checklist&inquiry_type=rfq" className="mt-[24px] w-full">Send Tread Schedule</Button></aside>
        </div>
      </section>
      ...
      <section className="bg-white py-[55px] md:py-[89px]">
        <div className="site-container grid gap-[34px] lg:grid-cols-[.92fr_1.08fr] lg:items-start">
          ...
          <div><SectionTag>When a Cover Is Not Enough</SectionTag><h2 className="mt-[13px] text-[clamp(24px,3vw,34px)] font-extrabold leading-[1.15] text-t1">Replace the tread—or coordinate the complete stair</h2><p className="mt-[13px] text-f18 leading-golden text-t2">If the tread has lost section, the connection is unreliable, drainage is essential or the stair is new, move to a complete grating tread.</p><div className="mt-[24px] grid gap-[13px] sm:grid-cols-2">{[["Molded tread", "Mesh, depth, cut, nosing, bearing and clip/end-plate detail."], ["Pultruded T-bar", "Bearing direction, series, span, loads, deflection and end plates."], ["Handrail interface", "Stair slope, rail height, returns, posts and load basis."], ["Release documents", "Schedule, drawing, load table, resin/surface spec and hardware BOM."]].map(([title, body]) => <article key={title} className="rounded-[8px] border border-border-default bg-bg2 p-[18px]"><h3 className="text-f16 font-bold text-t1">{title}</h3><p className="mt-[7px] text-f14 leading-golden text-t2">{body}</p></article>)}</div><div className="mt-[24px] flex flex-wrap gap-[10px]"><Button href="/products/molded-frp-grating" variant="secondary">Molded Grating Data</Button><Button href="/products/frp-gratings" variant="secondary">Pultruded T-Bar Data</Button></div></div>
        </div>
      </section>

      <section className="bg-bg2 py-[55px] md:py-[89px]">
        <div className="site-container"><SectionTag>Cover Installation Sequence</SectionTag><h2 className="mt-[13px] text-[clamp(24px,3vw,34px)] font-extrabold leading-[1.15] text-t1">Survey, dry-fit, fasten, inspect</h2><p className="mt-[13px] max-w-[930px] text-f16 leading-golden text-t2">This is a selection-stage overview. The order drawing controls fastener material, holes, spacing, adhesive if used and cut-edge sealing.</p><ol className="mt-[34px] grid gap-[13px] md:grid-cols-2 lg:grid-cols-4">{installationSteps.map(([title, body], index) => <li key={title} className="rounded-[8px] border border-border-default bg-white p-[21px]"><span className="flex h-[32px] w-[32px] items-center justify-center rounded-full bg-deep text-f12 font-bold text-white">{index + 1}</span><h3 className="mt-[13px] text-f18 font-bold text-t1">{title}</h3><p className="mt-[9px] text-f14 leading-golden text-t2">{body}</p></li>)}</ol></div>
      </section>
