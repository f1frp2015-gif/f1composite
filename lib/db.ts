...
async function ensureTable(sql: NonNullable<ReturnType<typeof getSql>>) {
  ...
  // This database is shared with f1frp/getfrp, which already owns a public
  // `inquiries` table with a different UUID/user-to-user schema. Keep F1's
  // website leads in a namespaced table so CREATE TABLE IF NOT EXISTS cannot
  // silently bind this app to that unrelated table.
  //
  // The identity starts above the ids issued by the former Neon-backed table.
  // Cockpit persists the website id as its local primary key and uses it as a
  // high-water cursor, so reusing ids after the database move would make fresh
  // leads look old (or collide with the local mirror).
  await sql`
    CREATE TABLE IF NOT EXISTS f1composite_inquiries (
  ...
  `;
  await sql`CREATE INDEX IF NOT EXISTS idx_f1composite_inquiries_id ON f1composite_inquiries (id)`;
  // L2 closed-loop instrumentation (win/loss/margin) — additive, nullable, idempotent.
  await sql`ALTER TABLE f1composite_inquiries ADD COLUMN IF NOT EXISTS outcome         TEXT`;        // won | lost | pending | null
  await sql`ALTER TABLE f1composite_inquiries ADD COLUMN IF NOT EXISTS outcome_at      TIMESTAMPTZ`;
  await sql`ALTER TABLE f1composite_inquiries ADD COLUMN IF NOT EXISTS realized_margin NUMERIC`;     // gross margin % on won deals
  await sql`ALTER TABLE f1composite_inquiries ADD COLUMN IF NOT EXISTS loss_reason     TEXT`;        // price | lead_time | spec | trust | other | null
  await sql`ALTER TABLE f1composite_inquiries ADD COLUMN IF NOT EXISTS outcome_note    TEXT`;
  tableReady = true;
}
...
export async function insertInquiry(input: InquiryInput): Promise<number | null> {
  ...
  const rows = await sql`
    INSERT INTO f1composite_inquiries
...
export async function markInquiryEmailed(id: number): Promise<void> {
  const sql = getSql();
  if (!sql) return;
  await sql`UPDATE f1composite_inquiries SET email_sent = true WHERE id = ${id}`;
}
...
export async function inquiryMetrics(opts: { weeks?: number } = {}): Promise<InquiryMetrics | null> {
  ...
  const lossRows = (await sql`
    ...
    FROM f1composite_inquiries WHERE outcome = 'lost'
  ...
  const countryRows = (await sql`
    SELECT country, count(*)::int AS total,
           count(*) FILTER (WHERE outcome = 'won')::int  AS won,
           count(*) FILTER (WHERE outcome = 'lost')::int AS lost
    FROM f1composite_inquiries GROUP BY country ORDER BY total DESC LIMIT 30
  ...
  const sourceRows = (await sql`
    SELECT source, count(*)::int AS total,
           count(*) FILTER (WHERE outcome = 'won')::int  AS won,
           count(*) FILTER (WHERE outcome = 'lost')::int AS lost
    FROM f1composite_inquiries GROUP BY source ORDER BY total DESC LIMIT 30
