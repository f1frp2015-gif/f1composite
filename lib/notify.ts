...
export const NOTIFY_EMAILS: string[] = (
  ...
  "Doris.li@f1composite.com,ori@f1composite.com"
)
  .split(",")
  .map((s) => s.trim())
  .filter(Boolean);

/** Verified Resend sender used for internal inquiry notifications. */
...
/**
 * Send a single notification to ALL {@link NOTIFY_EMAILS} (one email, every
 * mailbox receives it simultaneously). Never throws — returns `{ ok, error }`
 * so callers can log and degrade gracefully when Resend fails.
 */
