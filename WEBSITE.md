...
能源、工业化工、基础设施、海洋、交通、水处理 6 个行业页共用 `components/industries/IndustryPage.tsx`，内容都在 `content/data/industryPages.ts`，页面文件只剩 metadata。顺序固定：页头（场景图版、更新日期）→ 各区域用在哪里、要核对什么（表格）→ 产品（带截面图标的卡片）→ 案例和应用 → 文件 → 常见问题 → 询价（清单按行业写）。建筑行业页内容多（6 个应用区的产品目录和设计指南），保留自己的页面，只换成同一套页头、吸顶导航、段落和收尾询价区；页头图版就是带编号的建筑概念图，下面的产品组按同一编号排列，设计指南每节只展开第一段，其余点"Continue reading"展开。
